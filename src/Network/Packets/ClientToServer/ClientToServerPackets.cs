...
/// </summary>
public readonly struct ServerPlayerListRequest
{
    private readonly Memory<byte> _data;

    /// <summary>
    /// Initializes a new instance of the <see cref="ServerPlayerListRequest"/> struct.
    /// </summary>
    /// <param name="data">The underlying data.</param>
    public ServerPlayerListRequest(Memory<byte> data)
        : this(data, true)
    {
    }
    ...
    /// <param name="data">The underlying data.</param>
    /// <param name="initialize">If set to <c>true</c>, the header data is automatically initialized and written to the underlying span.</param>
    private ServerPlayerListRequest(Memory<byte> data, bool initialize)
    {
        this._data = data;
        if (initialize)
        {
    ...
    /// <summary>
    /// Performs an implicit conversion from a Memory of bytes to a <see cref="ServerPlayerListRequest"/>.
    /// </summary>
    /// <param name="packet">The packet as span.</param>
    /// <returns>The packet as struct.</returns>
    public static implicit operator ServerPlayerListRequest(Memory<byte> packet) => new (packet, false);

    /// <summary>
    /// Performs an implicit conversion from <see cref="ServerPlayerListRequest"/> to a Memory of bytes.
    /// </summary>
    /// <param name="packet">The packet as struct.</param>
    /// <returns>The packet as byte span.</returns>
    public static implicit operator Memory<byte>(ServerPlayerListRequest packet) => packet._data; 
}
